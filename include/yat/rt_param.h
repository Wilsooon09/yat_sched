 ...
 * priority boosting. YAT_MAX_PRIORITY denotes the maximum priority value
 * range.
 */
...
#define YAT_NO_PRIORITY	UINT_MAX
...
#define yat_is_valid_fixed_prio(p)		\
	((p) >= YAT_HIGHEST_PRIORITY &&	\
	 (p) <= YAT_LOWEST_PRIORITY)
...
/* don't export internal data structures to user space (libyat) */
#ifdef __KERNEL__
...
struct rt_param {
	...
	/* How many YAT^RT locks does the task currently hold/wait for? */
