...
void yat_fork(struct task_struct *tsk);
void yat_exec(void);
/* clean up real-time state of a task */
void yat_clear_state(struct task_struct *dead_tsk);
...
void yat_do_exit(struct task_struct *tsk);
...
/* Our notion of time within YAT: kernel monotonic time. */
 ...
 */
#define YAT_QUANTUM_LENGTH_NS (CONFIG_YAT_QUANTUM_LENGTH_US * 1000ULL)
...
{
	s64  quantum_length = YAT_QUANTUM_LENGTH_NS;
...
{
	return quanta * YAT_QUANTUM_LENGTH_NS;
}
