...
#define FP_PRIO_BIT_WORDS (YAT_MAX_PRIORITY / BITS_PER_LONG)

#if (YAT_MAX_PRIORITY % BITS_PER_LONG)
#error YAT_MAX_PRIORITY must be a multiple of BITS_PER_LONG
#endif

/* bitmask-inexed priority queue */
struct fp_prio_queue {
	unsigned long	bitmask[FP_PRIO_BIT_WORDS];
	struct bheap	queue[YAT_MAX_PRIORITY];
};
...
struct fp_ready_list {
	unsigned long     bitmask[FP_PRIO_BIT_WORDS];
	struct list_head  queue[YAT_MAX_PRIORITY];
};
