...
#define SCHED_TRACE_NAME "yat/log"

/* Compute size of TRACE() buffer */
#define YAT_TRACE_BUF_SIZE (1 << CONFIG_SCHED_DEBUG_TRACE_SHIFT)
...
static DEFINE_KFIFO(debug_buffer, char, YAT_TRACE_BUF_SIZE);
...
/*
 * log_fops  - The file operations for accessing the global YAT log message
 *             buffer.
 *
...
static struct miscdevice yat_log_dev = {
...
static int __init init_sched_trace(void)
{
	...
	return misc_register(&yat_log_dev);
}

static void __exit exit_sched_trace(void)
{
	misc_deregister(&yat_log_dev);
}
