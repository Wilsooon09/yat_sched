...
static void yat_ctrl_vm_close(struct vm_area_struct* vma)
{
...
static vm_fault_t yat_ctrl_vm_fault(struct vm_fault* vmf)
{
...
static struct vm_operations_struct yat_ctrl_vm_ops = {
	.close = yat_ctrl_vm_close,
	.fault = yat_ctrl_vm_fault,
};

static int yat_ctrl_mmap(struct file* filp, struct vm_area_struct* vma)
{
	int err = 0;
...
static long yat_ctrl_ioctl(struct file *filp,
	unsigned int cmd, unsigned long arg)
{
	long err = -ENOIOCTLCMD;

	/* YAT^RT syscall emulation: we expose YAT^RT-specific operations
	 * via ioctl() to avoid merge conflicts with the syscall tables when
	 * rebasing YAT^RT. While this is not the most elegant way to expose
	 * syscall-like functionality, it helps with reducing the effort
	 * required to maintain YAT^RT out of tree.
	 */

	union yat_syscall_args syscall_args;
	...
	case LRT_yat_lock:
		return sys_yat_lock(arg);

	case LRT_yat_unlock:
		return sys_yat_unlock(arg);
...
static struct file_operations yat_ctrl_fops = {
	.owner = THIS_MODULE,
	.mmap  = yat_ctrl_mmap,
	.unlocked_ioctl = yat_ctrl_ioctl,
};

static struct miscdevice yat_ctrl_dev = {
	.name  = CTRL_NAME,
	.minor = MISC_DYNAMIC_MINOR,
	.fops  = &yat_ctrl_fops,
};

static int __init init_yat_ctrl_dev(void)
{
	int err;
	...
	printk("Initializing YAT^RT control device.\n");
	err = misc_register(&yat_ctrl_dev);
	if (err)
		printk("Could not allocate %s device (%d).\n", CTRL_NAME, err);
	return err;
}

static void __exit exit_yat_ctrl_dev(void)
{
	misc_deregister(&yat_ctrl_dev);
}

module_init(init_yat_ctrl_dev);
module_exit(exit_yat_ctrl_dev);
