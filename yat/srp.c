...
static int lock_srp_semaphore(struct yat_lock* l)
{
	struct task_struct* t = current;
	struct srp_semaphore* sem = container_of(l, struct srp_semaphore, yat_lock);
...
static int unlock_srp_semaphore(struct yat_lock* l)
{
	struct task_struct* t = current;
	struct srp_semaphore* sem = container_of(l, struct srp_semaphore, yat_lock);
	int err = 0;
...
static int open_srp_semaphore(struct yat_lock* l, void* __user arg)
{
	struct srp_semaphore* sem = container_of(l, struct srp_semaphore, yat_lock);
	int err = 0;
	struct task_struct* t = current;
...
static int close_srp_semaphore(struct yat_lock* l)
{
	struct srp_semaphore* sem = container_of(l, struct srp_semaphore, yat_lock);
	int err = 0;
...
static void deallocate_srp_semaphore(struct yat_lock* l)
{
	struct srp_semaphore* sem = container_of(l, struct srp_semaphore, yat_lock);
...
static struct yat_lock_ops srp_lock_ops = {
