...
void resched_curr(struct rq *rq)
{
	...
	if (cpu == smp_processor_id()) {
		...
		/* Cannot call set_tsk_need_resched() on YAT^RT task
...
void scheduler_ipi(void)
{
	...
	/* Let YAT^RT preemption state machine know about this IPI. */
	sched_state_ipi();
...
static void ttwu_queue(struct task_struct *p, int cpu, int wake_flags)
{
	...
	/*
	 * YAT^RT: plugins determine whether to send an IPI to the remote
	 * CPU.
	 */
...
//			printk(KERN_EMERG "try to wake up MrsP task %d. state %d, wake flag %d. \n", p->pid, state, wake_flags);
	...
	/* YAT^RT: once the task can be safely referenced by this
	 * CPU, don't mess with Linux load balancing stuff.
	 */
	if (is_realtime(p))
		goto yat_out_activate;
...
static void __sched notrace __schedule(bool preempt)
{
	...
	this_cpu_write(yat_preemption_in_progress, preempt);
	...
	this_cpu_write(yat_preemption_in_progress, false);
...
recheck:
	...
	if ((dl_policy(policy) && !__checkparam_dl(attr)) ||
		...
		return -EINVAL;
	if (policy == SCHED_YAT && policy == p->policy)
		return -EINVAL;
